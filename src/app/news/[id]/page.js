...
export default function NewsDetailPage() {
  ...
  try {
    const response = await axios.get(`${API_URL}/api/news?sort=date:desc&pagination[limit]=3&populate=image`, {
      headers: { Authorization: `Bearer ${API_TOKEN}` }
    });
    console.log('📦 Related news API response:', response.data);
    if (response.data?.data) {
...
useEffect(() => {
  console.log('🔄 Related news in state:', relatedNews);
}, [relatedNews]);
  ...
  <div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {relatedNews.map((item) => {
        ...
        console.log('🔍 Related item image structure:', item.attributes.image);
