    ...
    <div className="space-y-4">
      {items.map((item, index) => (
        <div key={index} className="border border-gray-200 rounded-lg overflow-hidden">
          ...
          {/* Accordion Content - FIXED IMAGE URL */}
          {openIndex === index && (
            <div className="p-6 border-t border-gray-200">
              {/* Image FIRST (if exists) - FIXED: removed apiUrl prefix */}
              {item.hasImage && item.image?.data?.attributes?.url && (
                <div className="mb-6">
                  <div className="relative h-[500px] w-full rounded-lg overflow-hidden bg-gray-100 shadow-lg">
                    <Image
                      // FIXED: Using the full URL directly from Strapi, not apiUrl + url
                     src={item.image.data.attributes.url?.startsWith('http') ? item.image.data.attributes.url : `${apiUrl}${item.image.data.attributes.url}`}
                      alt={item.title}
                      fill
