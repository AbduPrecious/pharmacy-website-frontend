...
export default function MobileMenu({ menus, getDropdownItems }) {
  ...
  return (
    <div className="md:hidden relative">
      ...
      <button
        onClick={toggleMenu}
        className="p-2 text-gray-700 hover:text-[#FFFF00] focus:outline-none transition-colors z-50 relative"
      ...
      {isOpen && (
        <div className="fixed inset-x-0 top-[72px] bottom-0 bg-white shadow-lg border-t border-gray-200 z-40 overflow-y-auto">
          <div className="py-2">
            {menus.map((menu) => {
                      ...
                      <div className="flex items-center justify-between w-full px-4 py-3">
                        ...
                        {(menu.attributes.title === 'Group Companies' || menu.attributes.title === 'R & D') ? (
                          ...
                          <Link
                            ...
                            className="text-gray-700 font-medium hover:text-[#FFFF00] transition-colors flex-1"
                            onClick={closeMenu}
                          >
                            {menu.attributes.title}
                          </Link>
