...
export default function Home() {
  ...
  const getMenus = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/menus?populate[parent]=*`, {
...
useEffect(() => {
  const loadData = async () => {
    try {
      const [menusData, headerData, slidesData, categoriesData, welcomeData, newsData, clientsData, footerData] = await Promise.all([
        getMenus(),
        getHeaderInfo(),
        getSlides(),
        getCategories(),
      ...
      if (slidesData?.data) setSlides(slidesData.data);
      ...
      if (footerData?.data) {
  console.log('✅ Footer data received:', footerData.data);
  setFooter(footerData.data);
  console.log('📦 Complete footerData:', JSON.stringify(footerData.data, null, 2));
} else {
  console.log('❌ No footer data received');
}
      setLoading(false);
    } catch (error) {
...
// Debug footer data - THIS IS THE SECOND CODE INCLUDED
useEffect(() => {
  if (footer) {
    console.log('🔍 Current footer state:', footer);
    console.log('🔍 Branch locations:', footer.branchLocations);
    console.log('🔍 Quick links:', footer.footerLinks);
    console.log('🔍 Social platforms:', footer.socialPlatforms);
    console.log('🔍 Copyright:', footer.copyrightText);
  } else {
    console.log('🔍 Footer state is null');
  }
}, [footer]);

  // Menu functions
  const getMainMenus = () => {
    return menus.filter(item => [1,2,3,4,5,6,7,8].includes(item.id));
  };

  const getDropdownItems = (menuId) => {
    if (menuId === 3) return menus.filter(item => [9,10,11,12].includes(item.id));
    if (menuId === 4) return menus.filter(item => [13,14,15,16].includes(item.id));
    if (menuId === 5) return menus.filter(item => [17,18,19,20].includes(item.id));
    if (menuId === 7) return menus.filter(item => [21,22,23,24].includes(item.id));
    return [];
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* TOP YELLOW BAR - Disappears when scrolled */}
      {!isScrolled && (
        <div className="bg-[#FFFF00] py-3 md:py-4 transition-all duration-300">
      ...
      {/* Add padding when navbar is fixed to prevent content jump */}
      {isScrolled && <div className="h-[72px]"></div>}

      {/* SLIDESHOW SECTION - Dynamic from Strapi */}
      {slides.length > 0 && (
        <div className="relative w-full h-[500px] overflow-hidden">
                ...
                <div className="flex h-full">
                  <div className="w-1/2 bg-black h-full flex items-center justify-center p-12">
                    <div className="text-white max-w-lg">
                      <h2 className="text-4xl font-bold mb-6 text-[#FFFF00]">{slide.attributes.title}</h2>
                      <p className="text-lg mb-8 leading-relaxed">{extractText(slide.attributes.description)}</p>
                  ...
                  <div className="w-1/2 h-full relative">
                    <Image
                      src={slide.attributes.image?.data?.attributes?.url ? `${API_URL}${slide.attributes.image.data.attributes.url}` : '/placeholder.jpg'}
          ...
          <button onClick={prevSlide} className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-3 rounded-full hover:bg-opacity-75 transition z-20">❮</button>
          <button onClick={nextSlide} className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-3 rounded-full hover:bg-opacity-75 transition z-20">❯</button>
          <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-3 z-20">
            {slides.map((_, index) => (
              <button key={index} onClick={() => goToSlide(index)} className={`w-3 h-3 rounded-full transition ${currentSlide === index ? 'bg-[#FFFF00] w-6' : 'bg-white bg-opacity-50 hover:bg-opacity-75'}`} />
            ))}
          </div>
        </div>
      )}
      ...
      {/* WELCOME SECTION - Dynamic from Strapi with FULL IMAGE */}
      {welcome && (
        <div className="bg-[#FFFF00] py-16">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex flex-col md:flex-row items-center gap-12">
              ...
              <div className="w-full md:w-1/2">
                {/* Made container TALLER - increased from h-[450px] to h-[550px] */}
      ...
      {/* CLIENTS SECTION - Dynamic from Strapi with BIGGER images */}
      {clients.length > 0 && (
...
<footer className="bg-gray-900 text-white">
  <div className="max-w-7xl mx-auto px-4 py-16">
    <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
      {/* Branch Locations - Now with .attributes */}
